...
const PROCESS_STEPS = [
  {
    ...
    desc: "Identify key business questions and objectives that guide dashboards, ensuring your visualizations deliver meaningful outcomes.",
    image: "https://images.unsplash.com/photo-1552664730-d307ca884978?auto=format&fit=crop&q=80&w=800&ixlib=rb-4.0.3"
  },
  {
    id: "02",
    title: "Collect & Consolidate Data",
    desc: "Gather data from multiple sources—ERP, CRM, spreadsheets, or databases—to unify your dashboard with accurate, structured information.",
  ...
  },
  {
    ...
    desc: "Explore datasets to identify patterns, trends, and insights. Analytics reveals actionable intelligence that guides informed decisions.",
    image: "https://images.unsplash.com/photo-1551434678-e076c223a692?auto=format&fit=crop&q=80&w=800&ixlib=rb-4.0.3"
  },
  {
    id: "04",
    title: "Select Visualization Techniques",
    desc: "Choose the most effective visualization methods—charts, maps, heatmaps, or storytelling layouts—to amplify insights and highlight what matters most.",
    image: "https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&q=80&w=800&ixlib=rb-4.0.3"
  },
  {
    id: "05",
    title: "Structure & Refine Data",
    desc: "Clean, format, and model datasets for accuracy and consistency. Structured data ensures dashboards load faster and deliver precise insights every time.",
    image: "https://images.unsplash.com/photo-1558494949-ef526b0042a0?auto=format&fit=crop&q=80&w=800&ixlib=rb-4.0.3"
  },
  {
    id: "06",
    title: "Build Your Visual Story",
    desc: "Create interactive dashboards that tell a clear story, making complex insights simple, memorable, and actionable across your organization.",
                ...
                <div className={`rounded-2xl overflow-hidden border shadow-2xl ${borderColor} ${isLight ? 'bg-white' : 'bg-[#0f172a]'}`}>
                    ...
                    <div className="p-6 md:p-8 grid gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            {[1,2,3,4].map(i => (
                                <div key={i} className={`h-24 rounded-xl border p-4 flex flex-col justify-between ${cardBg}`}>
                                    <div className="flex justify-between items-center">
                                       <div className={`h-3 w-16 rounded opacity-50 ${isLight ? 'bg-gray-300' : 'bg-white/20'}`}></div>
                                       <div className="w-6 h-6 rounded-full bg-green-500/20 text-green-500 flex items-center justify-center text-[10px] font-bold">↑</div>
                                    </div>
                                    <div className={`h-8 w-24 rounded ${isLight ? 'bg-gray-200' : 'bg-white/20'}`}></div>
                                </div>
                            ))}
                        </div>
                        {/* Charts Row */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-64">
                             <div className={`md:col-span-2 rounded-xl border ${cardBg} p-4 relative overflow-hidden group flex flex-col`}>
                                 <div className="font-bold text-sm mb-4">Revenue Trend</div>
                                 <div className="flex-1 flex items-end justify-between px-2 pb-2 gap-2 opacity-80">
                                     {[40, 60, 45, 70, 65, 80, 50, 75, 90, 85, 70, 95].map((h, idx) => (
                                         <div key={idx} style={{height: `${h}%`}} className={`flex-1 rounded-t-sm transition-all duration-1000 group-hover:bg-cyan-500 ${isLight ? 'bg-blue-300' : 'bg-blue-500'}`}></div>
                                     ))}
                                 </div>
                             </div>
                             <div className={`rounded-xl border ${cardBg} p-4 relative flex flex-col`}>
                                 <div className="font-bold text-sm mb-4">Distribution</div>
                                 <div className="flex-1 flex items-center justify-center relative">
                                     <svg viewBox="0 0 100 100" className="w-32 h-32 rotate-[-90deg]">
                                        <circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="20" fill="none" className={isLight ? 'text-gray-100' : 'text-gray-800'} />
                                        <circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="20" fill="none" strokeDasharray="180 251" className="text-cyan-500" />
                                        <circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="20" fill="none" strokeDasharray="50 251" strokeDashoffset="-180" className="text-purple-500" />
                                     </svg>
                                 </div>
                             </div>
                        </div>
                ...
                <div className="flex gap-6 overflow-x-auto pb-8 snap-x">
                    {['Sales Overview', 'Marketing Funnel', 'Product Usage', 'Customer Support'].map((t, i) => (
                        <div key={i} className={`snap-center shrink-0 w-72 h-48 rounded-2xl border p-4 flex flex-col justify-between transition-all hover:-translate-y-1 cursor-pointer group ${cardBg}`}>
                            <div className={`h-24 rounded-lg w-full mb-4 relative overflow-hidden flex items-end px-2 pb-0 gap-1 ${isLight ? 'bg-gray-100' : 'bg-white/5'}`}>
                                {[40,70,50,90,60].map((h, j) => (
                                   <div key={j} style={{height: `${h}%`}} className={`flex-1 rounded-t-sm ${isLight ? 'bg-blue-300' : 'bg-blue-500'}`}></div>
                                ))}
                            </div>
                            <div>
...
// ... sub components ... (BeforeAfterSlider, AINarratorPreview, SmartQueryDemo, UseCaseCard, TestimonialCard, PricingCard, NewsletterCTA)
// Reusing previously defined subcomponents, ensuring BeforeAfterSlider is included correctly in full file output above.
            ...
            <div className={`absolute inset-0 flex items-center justify-center ${isLight ? 'bg-white' : 'bg-[#0f172a]'}`}>
                ...
                <div className="w-full h-full p-4 md:p-8 grid grid-cols-3 gap-4">
                    <div className="col-span-2 space-y-4">
                        <div className={`h-32 rounded-lg ${isLight ? 'bg-blue-50' : 'bg-blue-500/10'} flex items-end px-4 pb-2 gap-2`}>
                            {[30,50,40,70,60,80,90].map((h, i) => <div key={i} style={{height: `${h}%`}} className={`flex-1 ${isLight ? 'bg-blue-400' : 'bg-blue-500'} rounded-t-sm`}></div>)}
                        </div>
                        <div className={`h-full rounded-lg ${isLight ? 'bg-purple-50' : 'bg-purple-500/10'} relative overflow-hidden`}>
                             <div className="absolute inset-0 flex items-center justify-center">
                                 <div className="w-32 h-32 rounded-full border-8 border-purple-500 border-t-transparent animate-spin-slow"></div>
                             </div>
                        </div>
                    </div>
                    <div className="space-y-4">
                        <div className={`h-20 rounded-lg ${isLight ? 'bg-gray-50' : 'bg-white/5'} flex items-center justify-center font-bold text-2xl`}>$42k</div>
                        <div className={`h-20 rounded-lg ${isLight ? 'bg-gray-50' : 'bg-white/5'} flex items-center justify-center font-bold text-2xl`}>+15%</div>
                        <div className={`h-full rounded-lg ${isLight ? 'bg-cyan-50' : 'bg-cyan-500/10'} p-2`}>
                            <div className="w-full h-full bg-cyan-500/20 rounded"></div>
                        </div>
                    </div>
                </div>
